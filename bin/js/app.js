...
},{"./lib/helpers":2,"lasso-js":4,"page":5,"space-pen":6}],2:[function(require,module,exports){
(function() {
  var $, lasso, page;
...
},{"lasso-js":4,"page":5,"space-pen":6}],3:[function(require,module,exports){
...
/**
 * Lasso.js 0.0.1 Implementation of the mediator pattern.
 * (c) 2012 Max Podriezov.
 * Lasso.js can be distributed under the MIT license.
 */
(function (global) {

    //shortcut references and native methods
    ...
    //regex for mediator event names, could be comma separated words
    var _eventRgx = /^\w+([-_]*,*\w+)*$/;
    ...
    //Allows to publish and subscribe events with specified context.
    lasso.from = function (object) {
        _broadcaster = object;
        return this;
    };

    //Register event listener for specified event name/s.
    lasso.listen = lasso.subscribe = function (arg1, arg2, arg3, arg4) {
        var names, name, callback;

        if (_isStr(arg1)) {
            //if first arg is string assume args to be: channel names, callback, context, once flag
            if (!_eventRgx.test(arg1) || !_isFn(arg2)) throw "invalid params";

            //in case if events comma separated
            names = arg1.split(',');

            for (var channelIndex = 0; channelIndex < names.length; channelIndex++) {
                name = _trim(names[channelIndex]);
                if (!_channels[name]) {
                    _channels[name] = [];
                }
                //add subscription
                _channels[name].push({
                        once:arg4,
                        //null if was not set using 'from' method
            ...
            //assume args to be: object where key is channel name and value is function, context, once flag
            if (!_isObj(arg1)) throw "invalid params";
            for (name in arg1) {
                if (arg1.hasOwnProperty(name)) {
                    callback = arg1[name];
                    if (!_isFn(callback)) throw "not a function";
    ...
    //Register event listener for specified event name/s which can be executed only once.
    lasso.subscribeOnce = lasso.listenOnce = function () {
        ...
        lasso.listen.apply(this, args);
    };

    lasso.broadcast = lasso.publish = function (channels /*args*/) {
        var channelName, subscription, i, l, args;
        if (_isStr(channels)) {
            if (!_eventRgx.test(channels)) throw "invalid params";
            channels = channels.split(',');
            for (var channelIndex = 0; channelIndex < channels.length; channelIndex++) {
                channelName = _trim(channels[channelIndex]);
                if (!_channels[channelName]) return;

                //for each subscriptions
                for (i = 0, l = _channels[channelName].length; i < l; i++) {
                    subscription = _channels[channelName][i];
                    if (subscription.callback) {
                        args = slice.call(arguments, 1);
                        //subscribed only for broadcaster
                        if (subscription.broadcaster) {
                            if (_broadcaster === subscription.broadcaster) {
        ...
        } else if (_isObj(channels)) {
            for (channelName in channels) {
                if (channels.hasOwnProperty(channelName)) {
                    channelName = _trim(channelName);
    ...
    //removes all
    lasso.clear = function (names, handler) {
        if (!arguments.length) {
            _channels = {};
            return;
        }
        if (!_eventRgx.test(names)) throw "invalid 1 param";
        names = names.split(',');
        var toRemove = [];
        for (var nameIndex = 0; nameIndex < names.length; nameIndex++) {
            var channelName = _trim(names[nameIndex]);
            if (_channels[channelName]) {
                for (var subIndex = 0; subIndex < _channels[channelName].length; subIndex++) {
                    var subscription = _channels[channelName][subIndex];
                    if (_isFn(handler) && subscription.callback === handler) {
                        _arrayRemove(_channels[channelName], subIndex, subIndex)
                    } else {
        ...
        for (var rIndex = 0; rIndex < toRemove.length; rIndex++) {
            delete _channels[toRemove[rIndex]];
        }
    };

    //integrate mediator into object
    lasso.integrate = lasso.installInto = lasso.into = function (objs) {
        if (!_isObj(objs)) throw "cannot install into" + typeof objs;
        objs = _isArray(objs) ? objs : [objs];
        for (var objIndex = 0; objIndex < objs.length; objIndex++) {
            var o = objs[objIndex];
            o.broadcast = lasso.broadcast;
            o.listen = lasso.listen;
            o.publish = lasso.broadcast;
            o.subscribe = lasso.listen;
            o.from = lasso.from;
        }
        return lasso;
    };

    lasso.subscribers = lasso.listeners = function (names) {
        var handlers = [], subscription, subIndex, channelName;
        //all handlers
        if (!names) {
            for (channelName in _channels) {
                var channel = _channels[channelName];
                for (subIndex = 0; subIndex < _channels[channelName].length; subIndex++) {
                    subscription = channel[subIndex];
                    handlers.push(subscription.callback);
                }
            }
            //handelers with names
        } else {
            if (!_eventRgx.test(names)) throw "invalid 1 param";
            names = names.split(',');
            for (var nameIndex = 0; nameIndex < names.length; nameIndex++) {
                channelName = _trim(names[nameIndex]);
                if (_channels[channelName]) {
                    for (subIndex = 0; subIndex < _channels[channelName].length; subIndex++) {
                        subscription = _channels[channelName][subIndex];
                        handlers.push(subscription.callback);
                    }
                }
            }
    ...
    //returns true if passed object is Function
    function _isFn(o) {
        return _is(o, "function");
    }

    //return true if passed parameter is Object
    function _isObj(o) {
        return _is(o, "object");
    }

    //returns true if passed object has type of Array
    function _isArray(o) {
        return _is(o, "array");
    }
    ...
    //trim string method
    function _trim(str) {
        return nativeTrim ? nativeTrim.call(str) : str.replace(trimRgx, '');
    }

    // Array Remove - By John Resig (MIT Licensed)
    function _arrayRemove(array, from, to) {
        var rest = array.slice((to || from) + 1 || array.length);
        array.length = from < 0 ? array.length + from : from;
        return array.push.apply(array, rest);
    }

    // Export the 'lasso' object for Node.js, with backwards-compatibility for the old require() API.
    // If we're in the browser, add 'lasso' as a global object via a string identifier, for Closure Compiler "advanced" mode.
    if (typeof exports !== 'undefined') {
        if (typeof module !== 'undefined' && module.exports) {
            exports = module.exports = lasso;
        }
        exports.lasso = lasso;
    } else {
        global.lasso = lasso;
    ...
    //add to jQuery/Backbone if exist
    if (global.jQuery) $.lasso = jQuery.lasso = lasso;
    if (global.Backbone) global.Backbone.Mediator = global.Backbone.Lasso = lasso;
})(this);
...
},{"../vendor/jquery":9,"underscore-plus":7}],7:[function(require,module,exports){
(function (process){
(function() {
...
},{"oMfpAn":3,"underscore":8}],8:[function(require,module,exports){
//     Underscore.js 1.6.0
